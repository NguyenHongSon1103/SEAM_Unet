...
if __name__ == '__main__':
    ...
    parser.add_argument("--voc12_root", default='/data/sonnh8/SEAM/VOCdevkit/VOC2012', type=str)
...
#     train_dataset = voc12.custom_data.PolypImageDataset(transform=transforms.Compose([
#                         imutils.RandomResizeLong(448, 768),
#                         transforms.RandomHorizontalFlip(),
#                         transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),
#                         np.asarray,
...
#             print(cam_rv1.cpu().detach().numpy().shape)       
...
#                eq_mask = (torch.max(torch.abs(cam1-cam2),dim=1,keepdim=True)[0]<0.7).float()
            ...
            loss_ecr1 = torch.mean(torch.topk(tensor_ecr1.view(ns,-1), k=(int)(num_class*hs*ws*0.2), dim=-1)[0])
            loss_ecr2 = torch.mean(torch.topk(tensor_ecr2.view(ns,-1), k=(int)(num_class*hs*ws*0.2), dim=-1)[0])
